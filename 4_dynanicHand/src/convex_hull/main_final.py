...
def main():
    ...
    try:
        ...
        print(f"  [錯誤] 模型載入失敗: {e}")
        # 如果你有正確放 models/enhanced_model.h5，請修改上面的 MODEL_PATH
        # 暫時為了不讓程式崩潰，這裡可以 try-catch，但最好確保檔案存在
        sys.exit(1)
    ...
    while True:
        ...
        if has_person:
            ...
            # 【修正1】使用 .landmarks 屬性傳入預測
            hand_result = detector.detect(clean_frame) # 確保這裡是 detect 不是 find_hands
            
            gesture_obj = None # 這是物件 (給 tracker 用)
            gesture_int = 0    # 這是整數 (給邏輯判斷用)
            gesture_name = "idle"
            
            if hand_result:
                # 取得 DNN 預測
                # 【修正1】從 hand_result 取出 landmarks 陣列
                ...
                # 【修正2】從物件中取出整數 ID
                # 根據 DEBUG 訊息，正確屬性是 class_id
                if hasattr(gesture_obj, 'class_id'):
                    gesture_int = gesture_obj.class_id
                else:
                    # 如果真的還抓不到，才印 DEBUG
                    print(f"DEBUG: 無法找到 ID, 物件屬性: {dir(gesture_obj)}")
                    gesture_int = 0

                # 取得手勢名稱
                from config import GESTURE_NAMES
                gesture_name = GESTURE_NAMES.get(gesture_int, "Unknown")
                
                # 更新復健計數器 (Tracker 通常吃物件)
            ...
            if hull is not None:
                ...
                # --- 雙重驗證邏輯 ---
                # 【修正3】這裡使用取出的整數 gesture_int 來做判斷
            ...
            #     print(hand_result.landmarks)

            #     # 取得 DNN 預測
            #     raw_prediction = classifier.predict(hand_result.landmarks)
            #     gesture_id = smoother.smooth(raw_prediction)
            #     # 這裡假設你的 config.py 裡有 GESTURE_NAMES
            #     from config import GESTURE_NAMES
            #     gesture_name = GESTURE_NAMES.get(gesture_id, "Unknown")
                
            #     # 更新復健計數器
            #     stretch_record = tracker.update(gesture_id)
            #     if stretch_record:
            #         print(f"動作完成: {stretch_record.stretch_type}, 次數: {tracker.get_stats().total_count}")
            ...
            #     # 判定物理狀態 (Physical State)
            #     # 解決轉向問題: 只要 Solidity 高 (實心) 或 Circularity 高 (很圓)，都算握緊
            #     is_physically_closed = (solidity > 0.76) or (circularity > 0.65)
            #     is_physically_open = (solidity < 0.65) and (circularity < 0.5)

            #     # --- 雙重驗證邏輯 ---
            #     # 1. DNN 說是拳頭 (ID: 6, 1, 2, 3)，但 CV 發現形狀鬆散
            #     dnn_says_closed = gesture_id in [1, 2, 3, 6] # hook, angry, thumb, fist
            #     if dnn_says_closed and not is_physically_closed:
            #         warning_msg = "Warning: Grip not tight!"
            #         cv_feedback = "Tighten Grip!"
                
            #     # 2. DNN 說是伸展 (ID: 7)，但 CV 發現是圓的
            #     dnn_says_open = (gesture_id == 7) # spend_hand
            #     if dnn_says_open and is_physically_closed:
            #         warning_msg = "Warning: Hand not open!"
            #         cv_feedback = "Open Hand!"

                # 繪製握緊進度條 (基於 Solidity)
                # 將 0.6~0.85 映射為 0~100%
                bar_h = 200
        ...
        # 注意: 我們傳入 display_frame，這是已經畫上 CV 資訊的圖
        if has_person and hand_result:
            final_frame = renderer.render(
                display_frame, # 使用已疊加 CV 資訊的圖
                hand_result,
                gesture_int,
                tracker.get_stats(),
