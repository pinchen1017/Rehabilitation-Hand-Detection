"""
手部復健偵測系統 - v18 雙重驗證防抖動版
整合目標:
1. 將 main_convex15.py (v14) 的幾何核心植入 main_final3.py。
2. 實作「類別組合驗證」：DNN 與 CV 必須一致才輸出，否則視為 IDLE。
3. 實作「防抖動緩衝」：錯誤或轉場需持續 2 秒才生效，期間維持原狀態。
"""
...
class CVAnalyzer:
    """處理 YOLO 去背與 Convex Hull 幾何分析 (移植自 main_convex15.py)"""
    def __init__(self):
    ...
    def analyze_hand_geometry(self, clean_frame, hand_result):
        """計算幾何特徵 (Solidity, Defects, AR) - v14 邏輯"""
...
def main():
    ...
    STABILITY_THRESHOLD = 2.0  # 緩衝 2 秒
    last_valid_gesture = None  # 備份用
    ...
    while True:
        ...
        if has_person:
            ...
            if hand_result is not None:
                ...
                if hull is not None:
                    ...
                    elif aspect_ratio >= 1.60: cv_group = "LONG"  # 優先判斷長條
                    elif solidity > 0.82: cv_group = "ROUND"      # 次判斷握拳
                    ...
                    # 只有當兩者匹配時，才更新 instant_id
                    if is_match:
                        instant_id = dnn_id
                        last_valid_gesture = gesture # 備份正確的物件
        ...
        if final_id_to_tracker != 0:
            # 構造一個帶有正確 ID 的 gesture 物件送給 Tracker
        ...
        cv2.putText(display_frame, f"System: {debug_status}", (10, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
