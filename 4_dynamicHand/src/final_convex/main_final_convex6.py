"""
手部復健偵測系統 - v21 最終修正整合版
修正內容:
1. 修復 ValueError: 確保 analyze_hand_geometry 回傳 6 個值 (含 aspect_ratio)。
2. 解決辨識變差: 強制使用 '瘦身版' 骨架 (Line=10, No Close)，避免指縫被填滿。
3. 整合功能: 雙重驗證 (DNN+CV) + 防抖動 (Debounce)。
"""
...
class CVAnalyzer:
    ...
    def analyze_hand_geometry(self, clean_frame, hand_result):
        ...
        # 這能避免手指黏在一起，解決 integration 變差的問題
        ...
        for conn in finger_connections:
            for i in range(len(conn)-1):
                ...
                if 0 <= pt1[0] < roi_img.shape[1] and 0 <= pt1[1] < roi_img.shape[0]:
                    cv2.line(mask_skeleton, pt1, pt2, 255, 10) # 瘦線
                    cv2.circle(mask_skeleton, pt1, 6, 255, -1) # 小圓
                    cv2.circle(mask_skeleton, pt2, 6, 255, -1)

        mask_final = cv2.bitwise_or(mask_skin, mask_skeleton)
        
        # 移除 aggressive closing，只做輕微處理
        ...
        aspect_ratio = float(rect_h) / rect_w if rect_w > 0 else 0 # 這裡計算了 AR
