"""
手部復健偵測系統 - v16 防抖動緩衝版 (修正 import math 錯誤)
新增功能:
1. 狀態緩衝機制 (State Buffering): 
   - 當判定為錯誤手勢或IDLE時，不會立刻中斷計數。
   - 系統會進入 "Holding" 狀態，維持原本的計數，給予使用者 2 秒的修正時間。
   - 只有當錯誤狀態持續超過 2 秒，系統才會正式切換/重置。
2. 解決手部短暫遮擋、旋轉或辨識跳動導致的進度歸零問題。
"""
...
def main():
    ...
    STABILITY_THRESHOLD = 2.0 # 需要維持 2 秒才會切換
    last_valid_gesture = None # 用來在緩衝期傳給 Tracker
    ...
    while True:
        ...
        if has_person:
            ...
            if hand_result is not None:
                ...
                if hull is not None:
                    ...
                    if is_match:
                        instant_id = dnn_id
                        last_valid_gesture = gesture # 存起來備用
        ...
        # 情境 B: 偵測到不同手勢 (或手消失/誤判) -> 開始計時，但暫時維持原判
            ...
            if elapsed < STABILITY_THRESHOLD:
                # --- 緩衝期 (Grace Period) ---
                # 雖然偵測到錯誤，但我們「假裝」還在做原本的動作
                # 這樣計數器就不會斷掉！
                final_id_to_tracker = stable_id 
                debug_status = f"Holding... {remaining:.1f}s"
                
                # 在畫面上顯示倒數條，提醒使用者「快修正回來！」
                bar_w = 200
                fill_w = int(bar_w * (remaining / STABILITY_THRESHOLD))
                cv2.rectangle(display_frame, (width//2 - 100, 50), (width//2 + 100, 70), (100, 100, 100), -1)
                cv2.rectangle(display_frame, (width//2 - 100, 50), (width//2 - 100 + fill_w, 70), (0, 165, 255), -1)
                cv2.putText(display_frame, "Fix Gesture!", (width//2 - 60, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
        ...
        # ==========================================
        # 更新 Tracker (使用防抖後的 ID)
        # ==========================================
        
        # 為了讓 Tracker 運作，我們需要一個 gesture 物件
        # 如果現在是 Holding 狀態，我們強制使用 last_valid_gesture 並把 ID 改成 stable_id
        gesture_to_send = None
        
        if final_id_to_tracker != 0:
            if last_valid_gesture:
                gesture_to_send = last_valid_gesture
                gesture_to_send.class_id = final_id_to_tracker # 強制覆寫 ID
            elif gesture:
                gesture_to_send = gesture
                gesture_to_send.class_id = final_id_to_tracker
        else:
            # 如果判定為 Idle，就送 Idle 給 Tracker 讓它暫停/重置
            if gesture:
                gesture.class_id = 0
                gesture_to_send = gesture
        ...
        # ==========================================
        # 繪製 UI (使用 display_frame)
        # ==========================================
        height, width = display_frame.shape[:2]
        
        # 1. 繪製進度條 (如果穩定 ID 不是 Idle)
        if stable_id != 0:
            bar_color = (0, 255, 0) # 綠色代表穩定
            if "Holding" in debug_status: bar_color = (0, 165, 255) # 橘色代表警告
        ...
        # 2. 顯示系統狀態
        cv2.putText(display_frame, f"State: {debug_status}", (10, height - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

        # 3. 呼叫 Renderer 畫最後的資訊 (次數等)
