"""
純幾何特徵測試儀 - 最終群組驗證版 (Pure CV Final)
目的: 驗證「群組化邏輯」是否能單靠物理特徵區分不同類型的手勢。
不使用 DNN，完全依賴幾何閾值。
"""
...
def main():
    print("="*50)
    print("純幾何特徵測試儀 - 最終群組驗證 (Pure CV Final)")
    print("不使用 DNN，驗證物理群組分類準確度")
    print("="*50)
    ...
    while True:
        ...
        if has_person and hand_result:
            ...
            if hull is not None:
                ...
                # ========================================================
                # 最終群組分類邏輯 (Final Group Classification)
                # 這模擬了 "DNN 給出分類後，CV 會不會同意"
                # ========================================================
                
                group_name = "Unknown"
                color = (200, 200, 200)

                # 1. Spread Group (張開組: Spread Hand)
                if defects >= 3 or (defects >= 2 and solidity < 0.65):
                    group_name = "Group: OPEN (Spread Hand)"
                    color = (0, 255, 0) # Green
                
                # 2. Long Solid Group (長條實心組: Thumb Flexion, Duck)
                # 特徵: 很長 (AR > 1.35) 且 實心 (Sol > 0.68)
                elif ar > 1.35 and solidity > 0.68:
                    group_name = "Group: LONG (Thumb/Duck)"
                    color = (255, 255, 0) # Cyan
                    
                # 3. Wide Solid Group (寬扁實心組: Straight Hand)
                # 特徵: 很寬 (AR < 0.95) 且 實心
                elif ar < 0.95 and solidity > 0.65:
                    group_name = "Group: WIDE (Straight)"
                    color = (0, 255, 255) # Yellow

                # 4. Round Solid Group (圓球實心組: Fist, Angry)
                # 特徵: 極度實心 (> 0.82)
                elif solidity > 0.82:
                    group_name = "Group: ROUND (Fist/Angry)"
                    color = (0, 0, 255) # Red
                
                # 5. C-Shape Group (中間組: Hook)
                # 特徵: 剩下的區域 (AR 1.0~1.35, Solidity 0.65~0.82)
                elif 0.95 <= ar <= 1.35 and 0.65 < solidity <= 0.82:
                ...
                for i, line in enumerate(info_text):
                    ...
                    cv2.putText(display_frame, line, (10, 80 + i*40), cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness)
