 ...
 └────────────────────┬───────────────────────────────────┘
     ...
     └──────────────────────┬─────────────────────┘
              ...
              └──────────────┬────────────────┘
                             ...
                             ▼
               ┌──────────────────────────────┐
               │ 6. Landmark 結構轉換（42 維） │
               │   lm_norm → flatten (21×2)   │
               │   → [x0,y0,...,x20,y20]      │
               └──────────────┬───────────────┘
                              │
                              ▼
┌───────────────────────────────────────────────────────────────┐
│ 7. 資料增強（避免過擬合）                                      │
│   - 微小旋轉 AugRotate (θ~N(0,π/10))                          │
│   - 高斯雜訊 Noise (σ=0.01)                                   │
│   - 水平翻轉 (依需要，但復健手勢建議不要翻左右手)              │
└──────────────────────┬─────────────────────────────────────────┘
                       │
                       ▼
        ┌──────────────────────────────────────────┐
        │ 8. 寫入 CSV (Fasko CNN 訓練需求格式)       │
        │   每列 = 42 維 Landmark + label_id       │
        │   shape = (N_samples, 43)                │
        └──────────────────────┬───────────────────┘
                               │
                               ▼
...
### 1-3 MediaPipe Hands → 21 Landmark
以影像取得手部關節點，再轉為「21 個手部關節的 3D 座標」做骨架輸入（v0–v20）【手勢偵測模組→關節座標→手勢辨識模組→軌跡處理→字元辨識】的後端流程如論文系統框圖所示。
```
import cv2
...
### 1-4 Landmark 正規化：以「手腕」為中心的座標正規化
* **WRIST 中心化**以手腕做為座標原點（v0 與 v9 的中點），可讓資料在空間中更平均、穩定。
* **WRIST → MCP 尺度歸一化**：根據手掌中心與指尖的距離進行尺度調整
* **旋轉對齊**：將手指對齊到一個標準方向，以降低旋轉誤差（把手心方向對齊 x 軸）
```
import numpy as np
