"""
手部復健偵測系統 - v23 指縫邏輯整合版
整合內容:
1. 系統架構: 基於 main_final_convex6.py (DNN + Tracker + 防抖動)。
2. 幾何核心: 移植 main_convex16.py 的「指縫優先」邏輯。
   - 5指 -> OPEN
   - 1指 -> WIDE / C-SHAPE
   - 0指 -> ROUND / LONG
"""
...
def main():
    print("=" * 50)
    print("手部復健偵測系統 - v23 指縫邏輯整合版")
    print("邏輯來源: main_convex16.py (指縫優先)")
    print("=" * 50)
            ...
            if hand_result is not None:
                ...
                if hull is not None:
                    ...
                    # 2. 2-3指 -> IDLE
                        ...
                        cv_group = "IDLE_PARTIAL"
                    
                    # 3. 優先判斷超長 (AR >= 1.6) -> Long
                    elif aspect_ratio >= 1.60:
                        cv_group = "LONG"
                        
                    # 4. 1指 -> Wide 或 Hook
                    elif defects == 1:
                        if aspect_ratio < 1.05 and solidity > 0.65:
                            cv_group = "WIDE" # Straight
                        elif 0.65 < solidity <= 0.82:
                            cv_group = "C-SHAPE" # Hook
                    
                    # 5. 0指 (Defects == 0) -> Round, Long, or Fallback
                    else: 
                        if solidity > 0.82:
                            cv_group = "ROUND" # Fist/Angry
                        elif aspect_ratio > 1.1 and solidity > 0.68:
                            cv_group = "LONG" # Duck/Thumb
                        # [注意] 這是 convex16 特有的 fallback:
                        elif 0.65 < solidity <= 0.82:
                            cv_group = "ROUND" # 原本是 C-Shape，現在設為 Round
                    ...
                    # Duck (5): 容許 Long, C-Shape, Round (彈性最大)
