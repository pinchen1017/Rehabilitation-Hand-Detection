"""
純幾何特徵測試儀 (v9 - 數據校正版 - 修復縮排)
根據使用者提供的真實截圖數據進行閾值校正。
"""
...
def main():
    print("="*50)
    print("純幾何特徵測試儀 (v9 數據校正版)")
    print("依據真實截圖數據調整判定邏輯")
    print("="*50)
    ...
    while True:
        ...
        if has_person and hand_result:
            ...
            if hull is not None:
                ...
                # 1. Spread Hand (特徵: 指縫多, Solidity < 0.7)
                if defects >= 3 or (defects >= 2 and solidity < 0.65):
                    guess = "Spread Hand (Open)"
                    color = (0, 255, 0)
                
                # 2. Thumb Flexion (特徵: 長寬比 AR > 1.3, Solidity 高)
                elif ar > 1.3 and solidity > 0.7:
                    guess = "Thumb Flexion (Long)"
                    color = (255, 255, 0)
                    
                # 3. Hook (特徵: Solidity 介於中間, AR 微長)
                elif 1.1 <= ar <= 1.3 and 0.65 < solidity < 0.8:
                    guess = "Hook (C-Shape)"
                    color = (0, 165, 255)
                
                # 4. Fist / Angry / Straight (特徵: AR 接近 1, Solidity 高, 無指縫)
                elif solidity >= 0.80 and defects <= 1:
                    guess = "Fist / Angry / Straight (Solid)"
                    color = (0, 0, 255)
                
                # 5. The Duck (特徵: 有 1-2 個指縫, Solidity 偏低)
                elif defects >= 1 and 0.60 < solidity < 0.75:
                     guess = "The Duck (Beak)"
                     color = (255, 0, 255)
