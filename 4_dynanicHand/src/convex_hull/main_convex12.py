"""
純幾何特徵測試儀 - v12 左手專用調校版 (Pure CV Tuned)
針對左手特有的 AR 偏移與 Solidity 變化進行微調。
不使用 DNN。
"""
...
def main():
    print("="*50)
    print("純幾何特徵測試儀 - v12 左手調校版")
    print("調整 Long, Straight, Round 的判定區間")
    print("="*50)
    ...
    while True:
        ...
        if has_person and hand_result:
            ...
            if hull is not None:
                ...
                # 2. 超長條組 (Priority High)
                # 解決 Duck11-2 (Sol 0.86) 被誤判為 Fist 的問題
                # 如果 AR 超過 1.46 (很長)，就算很實心，也一定是 Long Group
                elif ar > 1.46: 
                    group_name = "Group: LONG (Thumb/Duck)"
                    color = (255, 255, 0)

                # 3. 寬扁組 (Straight Hand)
                # 修正: 將 AR 上限從 0.95 放寬到 1.05
                # 解決 straight_hand11 (AR 0.99) 偵測失敗的問題
                elif ar < 1.05 and solidity > 0.65:
                    group_name = "Group: WIDE (Straight)"
                    color = (0, 255, 255)

                # 4. 握拳組 (Round) - 左手旋轉優化
                # 情境 A: 只要超級實心 (>0.82)，不管 AR 多少 (只要沒超過上面 1.46)，都是拳頭
                # 情境 B: 中度實心 (>0.76) 但形狀還算方 (<1.45)，也是拳頭
                # 這解決了 angry_fist10-1 (AR 1.42, Sol 0.78) 的問題
                elif solidity > 0.82 or (solidity > 0.76 and ar < 1.45):
                    group_name = "Group: ROUND (Fist/Angry)"
                    color = (0, 0, 255)

                # 5. 一般長條組 (Duck/Thumb 剩下的)
                # 解決 the_duck11-3 (Sol 0.72) 的問題
