"""
純幾何特徵測試儀 - v14 最終嚴格版 (Pure CV Final v14)
依據最後兩張關鍵截圖進行邏輯鎖定：
1. Image 1 (Sol 0.88, AR 1.44) -> ROUND
2. Image 2 (Sol 0.87, AR 1.64) -> LONG
修正策略: 優先判定 AR > 1.60 為 Long，再判定高 Solidity 為 Round。
"""
...
def main():
    print("="*50)
    print("純幾何特徵測試儀 - v14 最終嚴格版")
    print("邏輯: 優先判定 AR > 1.6 為 Long，區分高實心度的 Round 與 Long")
    print("="*50)
    ...
    while True:
        ...
        if has_person and hand_result:
            ...
            if hull is not None:
                ...
                # 2. 超級長條組 (Absolute Long) - 優先權最高！
                # 解決 Image 2: Sol 0.87, AR 1.64 -> 必須是 Long
                # 只要 AR >= 1.60，不管多實心，都是 Long
                elif ar >= 1.60: 
                    group_name = "Group: LONG (Thumb/Duck)"
                    color = (255, 255, 0) # Cyan

                # 3. 握拳組 (Round) - 左手旋轉優化
                # 解決 Image 1: Sol 0.88, AR 1.44 -> 必須是 Round
                # 因為 AR < 1.60 (沒被上面抓走)，且 Sol > 0.82，所以判定為 Round
                elif solidity > 0.82:
                    group_name = "Group: ROUND (Fist/Angry)"
                    color = (0, 0, 255) # Red

                # 4. 寬扁組 (Straight Hand)
                # 維持 AR < 1.0 的嚴格限制，避免抓到 Hook
                elif ar < 1.0 and solidity > 0.65:
                    group_name = "Group: WIDE (Straight)"
                    color = (0, 255, 255) # Yellow

                # 5. 一般長條組 (Intermediate Long)
                # 處理那些不夠實心、或者沒那麼長的 Duck
