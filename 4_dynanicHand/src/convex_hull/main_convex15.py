"""
Hand Rehab System - v21 Final Integrated Version
Key Fixes & Features:
1. Fixes ValueError by ensuring analyze_hand_geometry returns all 6 values.
2. Uses 'Skinny Skeleton' drawing (Line=10, Circle=6, No aggressive closing) from v14/15 
   to ensure accurate defect detection (preventing fingers from merging).
3. Implements Double Verification: DNN and CV must match.
4. Implements Debounce Buffer: Prevents jitter by requiring state stability for 2 seconds.
"""
...
class CVAnalyzer:
    """
    Core Analyzer (Transplanted from main_convex15.py)
    Uses 'Skinny Skeleton' logic to ensure gaps between fingers are preserved
    for accurate defect counting.
    """
    def __init__(self):
        print("  [System] Initializing YOLOv8-Seg & CV Analyzer...")
    ...
    def analyze_hand_geometry(self, clean_frame, hand_result):
        ...
        Crucial Fix: Returns 6 values and uses skinny drawing parameters.
        """
        ...
        # 3. Skeleton Filling (Skinny Version: Line=10, Circle=6)
        # Prevents fingers from merging, fixing the integration issue.
        ...
        for conn in finger_connections:
            for i in range(len(conn)-1):
                ...
                if 0 <= pt1[0] < roi_img.shape[1] and 0 <= pt1[1] < roi_img.shape[0]:
                    cv2.line(mask_skeleton, pt1, pt2, 255, 10) # Skinny line
                    cv2.circle(mask_skeleton, pt1, 6, 255, -1) # Small circle
                    cv2.circle(mask_skeleton, pt2, 6, 255, -1)

        mask_final = cv2.bitwise_or(mask_skin, mask_skeleton)
        
        # Only mild morphological operations, NO aggressive closing
        ...
        # FIXED: Now returning 6 values correctly
        return hull_global, solidity, circularity, defects_count, aspect_ratio, debug_mask

def main():
    print("=" * 50)
    print("Hand Rehab System - v21 Final Corrected Integration")
    print("=" * 50)
    ...
    print("\nSystem Started - Press 'q' to exit, 'r' to reset")
    ...
    while True:
        ...
        if has_person:
            ...
            if hand_result is not None:
                ...
                # 2. CV Geometric Validation (Now returns correct 6 values)
                ...
                if hull is not None:
                    ...
                    # === A. Geometric Classification (v14 Golden Rules) ===
                    cv_group = "UNKNOWN"
                    ...
                    if is_match:
                        ...
                        instant_id = 0 # Invalid match -> IDLE
        else:
        ...
        if instant_id == stable_id:
            ...
            if elapsed < STABILITY_THRESHOLD:
                ...
                # Draw countdown bar on main frame
                bar_w = 200
        ...
        if gesture_to_send:
            ...
            if stretch_record:
                 print(f"Completed! {stretch_record.stretch_type}")

        # === E. Render UI ===
        cv2.putText(debug_mask_display, f"CV: {cv_group_name}", (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        cv2.putText(debug_mask_display, f"Sys: {debug_status}", (10, 60), 
